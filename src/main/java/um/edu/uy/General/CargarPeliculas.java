...
import um.edu.uy.entities.ColeccionInfo;
...
public class CargarPeliculas {
    ...
    private final HashTable<Integer, ColeccionInfo> colecciones;
    ...
    public void cargarDatos() throws IOException, CsvValidationException, InvalidIndex, ValueNoExists {
        ...
        while ((dataLine = lectorCSV.readNext()) != null) {
            ...
            ColeccionInfo coleccion = searchColecciones(dataLine[1], idPelicula, dataLine[8]);
            if (coleccion != null){
                try {
    ...
    public HashTable<Integer, ColeccionInfo> getColecciones() {
        return colecciones;
    }
    ...
    private ColeccionInfo searchColecciones(String entrada, int idPelicula, String nombrePelicula){
        if (entrada == null) {
            return null;
        } else if(entrada.trim().isEmpty()){
            return new ColeccionInfo(idPelicula, nombrePelicula);
        }
        ...
        if (matcher.find()){
            try {
                int id = Integer.parseInt(matcher.group(1));
                String nombre = matcher.group(2);
                return new ColeccionInfo(id, nombre);
            } catch (NumberFormatException ignored) {
                return null;
            }
        }
